...
export default function PayrollProcessing({ onPayrollStatusChange }: PayrollProcessingProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: numericFields.includes(name) ? value.replace(/[^\d.-]/g, "") || "0" : value,
    }))
    ...
    if (name === "employeeId") {
      ...
      if (selectedEmployee) {
        setFormData((prev) => ({
          ...
          basicSalary: selectedEmployee.salary?.toString() || "0",
        }))
      }
    }
  }
  ...
  const calculatePayroll = async () => {
    ...
    if (editingPayroll) {
      alert(
        "Recalculation is disabled during edit mode to preserve the original payroll values. If you need to recalculate, please save your changes first and create a new payroll entry.",
      )
      return
    }
  ...
  const calculatePayrollLocally = (): { grossPay: number; totalDeductions: number; netPay: number } => {
    const safeParseFloat = (value: string | number): number => {
      ...
      // Return 0 for any invalid numbers
    ...
    const overtimePay = roundToCents(overtimeHours * hourlyRate * 1.25) // 125% of regular hourly rate
    ...
    let grossPayCalculation = Number(basicSalary) || 0
    grossPayCalculation = Number(grossPayCalculation) + Number(overtimePay)
    grossPayCalculation = Number(grossPayCalculation) + Number(holidayPay)
    grossPayCalculation = Number(grossPayCalculation) + Number(nightDifferential)
    grossPayCalculation = Number(grossPayCalculation) + Number(salaryAdjustment)
    grossPayCalculation = Number(grossPayCalculation) - Number(absences)
    grossPayCalculation = Number(grossPayCalculation) - Number(lateDeductions)

    const grossPay = roundToCents(grossPayCalculation)

    let deductionsCalculation = Number(sssContribution) || 0
    deductionsCalculation = Number(deductionsCalculation) + Number(philhealthContribution)
    deductionsCalculation = Number(deductionsCalculation) + Number(pagibigContribution)
    deductionsCalculation = Number(deductionsCalculation) + Number(withholdingTax)

    const totalDeductions = roundToCents(deductionsCalculation)

    // Calculate net pay
    const netPay = roundToCents(Math.max(0, Number(grossPay) - Number(totalDeductions)))
    ...
    if (
      ...
      console.error("WARNING: Invalid calculation results detected!")
      console.error("Form data:", formData)
      console.error("Calculated values:", { grossPay, totalDeductions, netPay })
      alert("Error in payroll calculation. Please check your input values.")
  ...
  const processPayroll = async () => {
    ...
    try {
      const payrollData = {
        ...
        workedHours: Number(formData.workedHours) || 0,
        overtimeHours: Number(formData.overtimeHours) || 0,
        holidayPay: Number(formData.holidayPay) || 0,
        nightDifferential: Number(formData.nightDifferential) || 0,
        salaryAdjustment: Number(formData.salaryAdjustment) || 0,
        absences: Number(formData.absences) || 0,
        lateDeductions: Number(formData.lateDeductions) || 0,
        sssContribution: Number(formData.sssContribution) || 0,
        philhealthContribution: Number(formData.philhealthContribution) || 0,
        pagibigContribution: Number(formData.pagibigContribution) || 0,
        withholdingTax: Number(formData.withholdingTax) || 0,
        grossPay: Number(calculatedValues.grossPay) || 0,
        totalDeductions: Number(calculatedValues.totalDeductions) || 0,
        netPay: Number(calculatedValues.netPay) || 0,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (payroll.status === "processed") {
      confirmMessage =
        "This payroll has been processed. Are you sure you want to delete it? This action cannot be undone."
    } else if (payroll.status === "completed") {
      confirmMessage =
        "⚠️ WARNING: This payroll has been completed. Deleting it may affect financial records and compliance. Are you absolutely sure you want to proceed?"
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payrollData = {
        ...formData,
        basicSalary: Number.parseFloat(formData.basicSalary) || 0,
        workedHours: Number.parseFloat(formData.workedHours) || 0,
        overtimeHours: Number.parseFloat(formData.overtimeHours) || 0,
        holidayPay: Number.parseFloat(formData.holidayPay) || 0,
        nightDifferential: Number.parseFloat(formData.nightDifferential) || 0,
        salaryAdjustment: Number.parseFloat(formData.salaryAdjustment) || 0,
        absences: Number.parseFloat(formData.absences) || 0,
        lateDeductions: Number.parseFloat(formData.lateDeductions) || 0,
        sssContribution: Number.parseFloat(formData.sssContribution) || 0,
        philhealthContribution: Number.parseFloat(formData.philhealthContribution) || 0,
        pagibigContribution: Number.parseFloat(formData.pagibigContribution) || 0,
        withholdingTax: Number.parseFloat(formData.withholdingTax) || 0,
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-green-600">
                        ₱
                        {(Math.round((calculatedValues.grossPay || 0) * 100) / 100).toLocaleString("en-US", {
                    ...
                    <div className="text-center">
                      <div className="text-3xl font-bold text-red-600">
                        ₱
                        {(Math.round((calculatedValues.totalDeductions || 0) * 100) / 100).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </div>
                    ...
                    <div className="text-center">
                      <div className="text-3xl font-bold text-blue-600">
                        ₱
                        {(Math.round((calculatedValues.netPay || 0) * 100) / 100).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </div>
                  ...
                  <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="text-sm text-blue-800">
                      ...
                      <ul className="mt-2 space-y-1">
                        <li>
                          • <strong>Save as Draft:</strong> Creates payroll with "Pending" status
                        </li>
                        <li>
                          • <strong>Process Payroll:</strong> Creates payroll with "Processed" status
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                {payrolls.map((payroll) => (
                  <tr key={payroll.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                      {payroll.cutoffStart ? new Date(payroll.cutoffStart).toLocaleDateString() : "N/A"} -{" "}
                      {payroll.cutoffEnd ? new Date(payroll.cutoffEnd).toLocaleDateString() : "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-black">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {payroll.status === "deleted"
                          ? "Deleted"
                          : (payroll.status || "pending").charAt(0).toUpperCase() +
                            (payroll.status || "pending").slice(1)}
                      </span>
                    </td>
                      ...
                      {payroll.status !== "deleted" && (
                        <>
                          <button
                            ...
                            title={
                              payroll.status === "completed" ? "Completed payrolls cannot be edited" : "Edit payroll"
                            }
